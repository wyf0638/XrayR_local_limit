...
func (c *APIClient) GetNodeInfo() (nodeInfo *api.NodeInfo, err error) {
	...
	switch c.NodeType {
	case "V2ray":
		nodeInfo, err = c.ParseV2rayNodeResponse(nil)
	default:
...
func (c *APIClient) GetUserList() (UserList *[]api.UserInfo, err error) {
	userList := make([]api.UserInfo, 1)
	for i := 0; i < 1; i++ {
		user := api.UserInfo{}
		user.UID = 101
		switch c.NodeType {
		case "V2ray":
			user.UUID = "0eeb38cf-70b9-77e0-3553-db9783be640e"
			user.Email = "test_2000@gmail.com"
			user.AlterID = 0
			// 单位Mbit
			user.SpeedLimit = 2 * 1000000 / 8
		}
		if c.SpeedLimit > 0 {
